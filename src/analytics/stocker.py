...
class Stocker():
    ...
    def __init__(self, ticker, asset_name = None, returns_df = None, number_of_years_back = 10, exchange='WIKI'):

        self.symbol = ticker
        if ticker:
            self.pqe = price_queries.engine()
            stock = self.pqe.get_historical_price_df(ticker)
            if ('AdjustedClose' not in stock.columns):
                stock['AdjustedClose'] = stock[helper.get_price_column_to_use_for_the_asset(ticker, stock, change_cols_to_titlecase=False)]
            stock['Adj. Open'] = stock['Open']
            # The starting price (starting with the opening price)
            self.starting_price = float(stock['Adj. Open'][0])
            # The most recent price
            self.most_recent_price = float(stock['AdjustedClose'][-1])
            stock['Daily Change'] = stock['AdjustedClose'] - stock['Adj. Open']
            stock['y'] = stock['AdjustedClose']
        else:
            self.symbol = asset_name
            stock = returns_df
            stock['y'] = stock['Returns']

        stock = stock[stock.index > helper.util.get_n_years_back_date(number_of_years_back)]
    ...
    # def plot_stock(self, start_date=None, end_date=None, stats=['AdjustedClose'], plot_type='basic'):
    ...
    def buy_and_hold(self, start_date=None, end_date=None, nshares=1):
        ...
        end_price = float(self.stock[self.stock['Date'] == end_date]['AdjustedClose'])

        # Make a profit dataframe and calculate profit column
        profits = self.make_df(start_date, end_date)
        profits['hold_profit'] = nshares * (profits['AdjustedClose'] - start_price)
    ...
    def changepoint_date_analysis(self, search=None):
        ...
        if not search:
            ...
            print(c_data.loc[:, ['Date', 'AdjustedClose', 'delta']][:5])
