...
def create_returns_tear_sheet(sravzid = None,
    ...
    plt.suptitle('%s returns as of %s. Start date %s End date %s.'%(sravzid,
    ...
    if upload_to_s3:
        return fig, f'{settings.constants.CONTABO_BUCKET_PREFIX}/assets/{sravzid}_monthly_returns.jpg'
    helper.save_plt(fig)
    return None, None
