...
from src.util import settings, helper, aws_cache, logger
import os, sys
...
class engine(object):
    ...
    def get_ts_analysis(self, sravzid, upload_to_aws=False, device=settings.constants.DEVICE_TYPE_PC):
        '''
            Performs ts analysis of assets
        '''
        price_df = self.pqe.get_historical_price_df(sravzid)
        col = helper.get_price_column_to_use_for_the_asset(sravzid, price_df)
        price_df = price_df[[col]]

        return self.get_ts_data(sravzid, price_df, col)

    def get_ts_data(self, sravzid, price_df, col):
