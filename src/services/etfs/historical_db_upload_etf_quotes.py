...
import datetime
from src.services.etfs.etf_historical_quotes import engine
...
def upload_historical_quotes(tickers=None):
    ...
    try:
        e = engine()
        e.get_eodhistoricaldata_historical_etf_quotes(
            tickers=tickers, upload_to_db=True, ndays_back=constants.NDAY_BACK_FOR_HISTORICAL_QUOTES)
    except Exception:
        _logger.error(
            'Could not upload historical ETF quotes', exc_info=True)
